...
@section('content')
    <div class="container" style="font-family: 'Mangal', Arial, sans-serif;">
        ...
        <form action="{{ route('income-source.update', $source->slug) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row mb-3">
                <div class="col-md-4 mb-2">
                    ...
                    <select name="category_id" class="form-control" required>
                        ...
                        @foreach ($sourceCategories as $row)
                            <option value="{{ $row->id }}"
                                {{ old('category_id', $source->category_id ?? '') == $row->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-2">
                    ...
                    <input type="text" name="source_name" class="form-control"
                        value="{{ old('source_name', $source->source_name ?? '') }}" placeholder="स्रोतको नाम लेख्नुहोस्">
                </div>
            </div>
